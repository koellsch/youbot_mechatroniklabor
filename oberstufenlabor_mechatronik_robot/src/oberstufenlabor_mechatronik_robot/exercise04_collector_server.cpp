/* *****************************************************************
 *
 * Oberstufenlabor Mechatronik I Aufgabe 4 - Roboterregelung
    ...
    collect_balls_server_(NULL)
{
    ...
    // === ROS COMMUNICATION ===
    circle_subscriber_ = subscribe("oberstufenlabor_mechatronik_vision/circle", 1, &exercise04_collector_server::circleCallback, this);
    ...
    timer_ = createTimer(ros::Duration(0.1), &exercise04_collector_server::timerCallback, this, false, false);

    // === START ACTION SERVER ===
    collect_balls_server_ = new CollectBallsServer(*this, "oberstufenlabor_mechatronik_robot/collect_balls", false);
    collect_balls_server_->registerGoalCallback(boost::bind(&exercise04_collector_server::collectBallsCallback, this));
    collect_balls_server_->registerPreemptCallback(boost::bind(&exercise04_collector_server::preemptCallback, this));
    ...
    ROS_INFO("Exercise04 Collector Server is initialised.");
}
...
bool exercise04_collector_server::gripRoutine()
{
    ...
    **************************************************************************************/
    ...
    ///<---
    //==================================================================================================================
    case GRIP_START: // initial state
    {
        ROS_INFO("Moving above grip pose...");
        arm_.moveToPose("GRIP_ABOVE");
        substate_ = ARM_ABOVE_GRIP;
        break;
    }
    //==================================================================================================================
    case ARM_ABOVE_GRIP: // move arm above grip pose
    {
        if(!arm_.isBusy())
        {
            ROS_INFO("Moving to grip pose...");
            arm_.moveToPose("GRIP", yapi::MotionMode::INTERPOLATE, yapi::InterpolationMode::CYLINDRIC);
            substate_ = ARM_TO_GRIP;
        }
        break;
    }
    //==================================================================================================================
    case ARM_TO_GRIP: // move arm to grip pose
    {
        if(!arm_.isBusy())
        {
            ROS_INFO("Closing gripper...");
            gripper_.setWidth(grip_width_);
            substate_ = CLOSE_GRIPPER;
        }
        break;
    }
    //==================================================================================================================
    case CLOSE_GRIPPER: // close the gripper
    {
        if(!gripper_.isBusy())
        {
            ROS_INFO("Moving above cargo slot...");
            arm_.moveToPose(current_cargo_slot_->above_pose);
            substate_ = ARM_ABOVE_CARGO;
        }
        break;
    }
    //==================================================================================================================
    case ARM_ABOVE_CARGO: // move arm above the cargo slot
    {
        if(!arm_.isBusy())
        {
            ROS_INFO("Moving down to cargo slot...");
            arm_.moveToPose(current_cargo_slot_->pose, yapi::MotionMode::INTERPOLATE);
            substate_ = ARM_DOWN_TO_CARGO;
        }
        break;
    }
    //==================================================================================================================
    case ARM_DOWN_TO_CARGO: // move arm down to the cargo slot
    {
        if(!arm_.isBusy())
        {
            ROS_INFO("Opening gripper...");
            gripper_.open();
            substate_ = OPEN_GRIPPER;
        }
        break;
    }
    //==================================================================================================================
    case OPEN_GRIPPER: // open the gripper to release the ball
    {
        if(!gripper_.isBusy())
        {
            ROS_INFO("Moving arm up from cargo slot...");
            arm_.moveToPose(current_cargo_slot_->above_pose, yapi::MotionMode::INTERPOLATE);
            substate_ = ARM_UP_FROM_CARGO;
        }

        break;
    }
    //==================================================================================================================
    case ARM_UP_FROM_CARGO: // move arm up from cargo
    {
        if(!arm_.isBusy())
            return true;
